...
The `children` association (`@OneToMany(fetch = FetchType.LAZY, @JoinColumn(name = "parent_id")`) is lazy-loaded. Code that needs to walk the full tree must call `loadLazyCollections()` first.
...
`WorkingDays` (`fineract-core/src/main/java/org/apache/fineract/organisation/workingdays/domain/WorkingDays.java`) stores a recurrence rule string (e.g., `FREQ=WEEKLY;BYDAY=MO,TU,WE,TH,FR`) that defines which days of the week are working days. The `WorkingDaysUtil` helper provides `isWorkingDay(LocalDate, WorkingDays)` and `getOffSetTillNextWorkingDay()` utilities consumed by the loan scheduler whenever a repayment or disbursement date falls on a non-working day.
