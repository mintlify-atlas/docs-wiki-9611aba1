...
sequenceDiagram
    participant Client
    participant TenantFilter as TenantAwareBasic<br/>AuthenticationFilter
    participant AuthManager as AuthenticationManager<br/>(ProviderManager)
    participant UDS as TenantAwareJpa<br/>UserDetailsService
    participant DB as Tenant DB<br/>(m_appuser)
    participant SC as SecurityContext

    Client->>TenantFilter: HTTP request<br/>Fineract-Platform-TenantId: default<br/>Authorization: Basic <base64>
    TenantFilter->>TenantFilter: Load tenant → ThreadLocalContextUtil
    TenantFilter->>AuthManager: authenticate(UsernamePassword token)
    ...
    TenantFilter-->>Client: 200 OK (X-Notification-Refresh header)
```
