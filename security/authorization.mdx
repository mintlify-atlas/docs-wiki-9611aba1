...
| `ALL_FUNCTIONS` | — | Superuser wildcard (read + write) |
| `ALL_FUNCTIONS_READ` | — | Read-only wildcard |
| `ALL_FUNCTIONS_WRITE` | — | Write-only wildcard |
| `BYPASS_TWOFACTOR` | — | Skip the 2FA second factor |

The wildcard permissions (`ALL_FUNCTIONS`, `ALL_FUNCTIONS_READ`, `ALL_FUNCTIONS_WRITE`) short-circuit all individual permission checks. `hasAllFunctionsPermission()` is called first inside every `AppUser.hasPermissionTo()` invocation.
