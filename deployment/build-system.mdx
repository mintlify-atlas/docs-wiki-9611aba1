...
Additional workflows cover MariaDB (`build-mariadb.yml`), MySQL (`build-mysql.yml`), Cucumber integration tests (`build-cucumber.yml`), E2E tests (`build-e2e-tests.yml`), code quality (`build-quality-checks.yml`), and Docker publishing (`build-docker.yml`).
