...
stateDiagram-v2
    ...
    TRANSFER_IN_PROGRESS --> TRANSFER_ON_HOLD : rejectLoanTransfer()
    TRANSFER_IN_PROGRESS --> ACTIVE : withdrawLoanTransfer()
    TRANSFER_IN_PROGRESS --> ACTIVE : acceptLoanTransfer()
...
| `TRANSFER_ON_HOLD` | 304 | Transfer rejected by destination; loan held pending resolution |
...
`closeLoan` fires `REPAID_IN_FULL` → `CLOSED_OBLIGATIONS_MET(600)`. `closeAsRescheduled` fires `LOAN_RESCHEDULE` → `CLOSED_RESCHEDULE_OUTSTANDING_AMOUNT(602)` and posts a `MARKED_FOR_RESCHEDULING(7)` transaction.
