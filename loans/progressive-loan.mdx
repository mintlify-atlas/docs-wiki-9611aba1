...
The default implementation is `EMICalculatorImpl` (also in `fineract-progressive-loan`). It uses the standard annuity formula with configurable rounding (`installmentAmountInMultiplesOf`) and the `MathContext` from the currency configuration.
