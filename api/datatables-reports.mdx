...
The `@ReportService(type = {...})` annotation on each implementation is the registration mechanism. For example, `DatatableReportingProcessService` is annotated `@ReportService(type = {"Table", "Chart", "SMS"})` to handle all three standard report types.
