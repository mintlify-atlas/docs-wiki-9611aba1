...
`BatchExecutionException` (`fineract-core/src/main/java/org/apache/fineract/batch/service/BatchExecutionException.java`) carries the `BatchRequest` of the failed step so callers can identify which sub-request caused the rollback.
...
The `fineract-core` module registers ~60+ strategies in the private `CommandStrategyProvider.init()` method invoked from the constructor. A representative sample:
