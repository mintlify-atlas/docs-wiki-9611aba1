...
sidebarTitle: "Home"
description: "Engineering reference for Apache Fineract — a multi-tenant core banking platform on Spring Boot with REST APIs for loans, savings, and accounting."
---
...
graph TB
    subgraph Clients
        RC[REST Client]
        FC["fineract-client (OpenAPI Java Client)"]
        FF["fineract-client-feign (Feign Client)"]
    end

    subgraph API["API Layer (fineract-provider)"]
        JA["Jersey JAX-RS ApiResources"]
        BA["Batch API BatchApiResource"]
        SA["Spring Security Filter Chain"]
    end

    subgraph CMD["Command Bus (fineract-core / fineract-command)"]
        CP[CommandProcessingService]
        CH[CommandHandlers]
        MK["Maker-Checker Audit Log"]
    end
    ...
    subgraph Infra["Infrastructure (fineract-core)"]
        MT["Multi-Tenancy DataSourcePerTenant"]
        EV["External Events (Kafka / JMS)"]
        JB["Spring Batch COB Pipeline (fineract-cob)"]
